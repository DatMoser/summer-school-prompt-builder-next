...
export function PromptNode({ data, selected }: { data: PromptNodeData; selected?: boolean }) {
  ...
  return (
    <div className="relative group">
      <div
        className={`bg-gradient-to-br from-purple-600 to-purple-700 p-6 rounded-xl shadow-lg border border-purple-400/20 w-[500px] min-h-[400px] hover:shadow-purple-500/20 hover:shadow-xl transition-all ${selected ? 'ring-2 ring-blue-400' : ''
          }`}
      >
        ...
        {/* Prompt Sections - Expandable - FORCE RE-MOUNT WITH KEY */}
        <div key={`prompt-sections-${connectionsKey}-${immediateKey}-${deletedKey}-${data._updateTimestamp || 0}`} className="mb-4">
          {!hasAnyConnections ? (
            ...
            <div key={`connected-sections-${connectionsKey}-${immediateKey}-${deletedKey}-${data._updateTimestamp || 0}`}>
              <BasePromptSection>
              ...
              {connectedComponentsWithIds.map(component => {
                ...
                switch (component.type) {
                  case 'evidence-input':
                    return (
                      <EvidencePromptSection
                        key={`evidence-${component.id}-${connectionsKey}-${immediateKey}-${deletedKey}-${data._updateTimestamp || 0}`}
                  ...
                  case 'style-personalization':
                    return (
                      <StylePromptSection
                        key={`style-${component.id}-${connectionsKey}-${immediateKey}-${deletedKey}-${data._updateTimestamp || 0}`}
                        componentId={component.id}
                        highlighted={isHighlighted}
                      />
                    );
                  case 'personal-data':
                    return (
                      <PersonalDataPromptSection
                        key={`personal-${component.id}-${connectionsKey}-${immediateKey}-${deletedKey}-${data._updateTimestamp || 0}`}
                        componentId={component.id}
                        highlighted={isHighlighted}
                      />
                    );
                  case 'output-selector':
                    return (
                      <OutputSelectorPromptSection
                        key={`output-${component.id}-${connectionsKey}-${immediateKey}-${deletedKey}-${data._updateTimestamp || 0}`}
                        componentId={component.id}
                        highlighted={isHighlighted}
                      />
                    );
            ...
            <div className="mb-3 text-xs">
              ...
              <div className="flex flex-wrap gap-1">
                {connectedComponentsWithIds.map(component => {
                  ...
                  return (
                    <span
                      key={`${component.id}-${connectionsKey}-${immediateKey}-${deletedKey}-${data._updateTimestamp || 0}`}
